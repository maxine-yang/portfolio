...
export default function Project({
  ...
  return (
    ...
    >
      <section className="bg-white max-w-[45rem] border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[28rem]  transition sm:group-even:pl-8 dark:text-white dark:bg-white/10 ">
        <div className="group pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col items-start gap-3 h-full sm:group-even:ml-[18rem]">
          <div className="flex flex-col gap-3 items-start ">
            <h3 className="text-2xl font-semibold group-hover:text-pink dark:group-hover:text-yellow hover:underline">
              <Link href={demoUrl} target="_blank">
                {activeLocale === "jp" ? title_jp : title}
              </Link>
            </h3>
          ...
          <p className="mt-2 leading-relaxed text-gray-700 dark:text-white/70">
            {activeLocale === "jp" ? desc_jp : description}
          </p>
